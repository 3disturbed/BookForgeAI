 ...
 * from raw markdown. Page boundaries in the model are respected; text reflows
 * within a page and overflow is reported rather than silently clipped.
 */
export async function renderPdf(model: PageModel): Promise<RenderResult> {
  ...
  return { pdf: Buffer.concat(chunks), pageCount, missingImages };
}
